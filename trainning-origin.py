...
def val(net, converter, dataset, max_iter=100):
    ...
    for i in range(max_iter):
        ...
        if pred.strip() == label:
            ...
            print(f"the pred is....{pred.strip()}...the true is....{label}")
        ...
        for j in range(n):
            ...
            if (j + 1) % interval == 0:
                ...
                if curAcc > acc:
                    ...
                    acc = curAcc
                    torch.save(checkpoint, f'{args.output_dir}/bestAcc.pth')
                    torch.save(model.state_dict(), 'crnn.pt')  # 只保存模型的结构
            pbar.update(j + 1, values=[('loss', loss / ((j + 1) * batchSize)), ('acc', curAcc)])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--imgW", type=int, default=100, help='img width')
    ...
    parser.add_argument("--epochs", type=int, default=9000, help='epochs')
